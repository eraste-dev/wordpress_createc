<?php
namespace WprAddonsPro\Modules\CategoryGridPro\Widgets;
...
class Wpr_Category_Grid_Pro extends Widget_Base {
	
	public function get_name() {
		return 'wpr-category-grid-pro';
	}

	public function get_title() {
		return esc_html__( 'Category Grid', 'wpr-addons' );
	}
	...
	public function get_keywords() {
		return [ 'post category grid', 'post categories'];
	}
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .wpr-grid-media-hover-bg' => 'mix-blend-mode: {{VALUE}}',
				],
				'separator' => 'after',
			]
		);
    ...
    public function register_controls() {
		...
		// Get the taxonomies from your method.
		$taxonomies = array_keys($this->get_post_taxonomies());
			...
			foreach ( $taxonomies as $taxonomy ) {
				...
				if ( !empty($terms) && !is_wp_error($terms) ) {
					...
					foreach ( $terms as $term ) {
						...
						// Get all fields of the term.
						// Check if ACF is active and get_fields function exists
						if (class_exists('acf') && function_exists('get_fields')) {
							$fields = get_fields($taxonomy . '_' . $term_id);
						} else {
							$fields = []; // Or handle the case when ACF isn't active as you need.
						}

						// Check if fields are not boolean and not empty.
						if (!is_bool($fields) && !empty($fields)) {
						// if ( !is_bool($fields) ) {
							// Change each value to be its key
		...
		// Flatten fields_array
		foreach ($fields_array as $item) {
			$flattened_array = array_merge($flattened_array, $item);
		}
		...
		$this->add_control(
			'thumbnail_term',
			[
				'label' => esc_html__( 'Select Custom Field For Image', 'wpr-addons' ),
				'type' => Controls_Manager::SELECT2,
				'label_block' => true,
				'default' => 'default',
				'options' => isset($flattened_array) ? $flattened_array : ''
			]
		);

		$this->add_control(
			'cat_grid_video_tutorial',
			[
				'type' => Controls_Manager::RAW_HTML,
				'raw' => __( 'How to add Category Thumbnails <ul><li><a href="https://www.youtube.com/watch?v=LqP_kIy_lpA&t=1s" target="_blank" style="color: #93003c;"><strong>Watch Video Tutorial <span class="dashicons dashicons-video-alt3"></strong></a></li></ul>', 'wpr-addons' ),
		...
		$this->add_control(
			'query_exclude_cats',
			[
				'label' => esc_html__( 'Exclude Categories', 'wpr-addons' ),
	...
	public function render_category_title( $settings, $class, $term ) {
		...
		// Pointer Class
		$categories_pointer = ! wpr_fs()->can_use_premium_code() || ! defined('WPR_ADDONS_PRO_VERSION') ? 'none' : $this->get_settings()['title_pointer'];
		$categories_pointer_animation = ! wpr_fs()->can_use_premium_code() || ! defined('WPR_ADDONS_PRO_VERSION') ? 'fade' : $this->get_settings()['title_pointer_animation'];
	...
	// public function render_category_title( $settings, $class, $term ) {
	// 	$title_pointer = ! wpr_fs()->can_use_premium_code() || ! defined('WPR_ADDONS_PRO_VERSION') ? 'none' : $this->get_settings()['title_pointer'];
	// 	$title_pointer_animation = ! wpr_fs()->can_use_premium_code() || ! defined('WPR_ADDONS_PRO_VERSION') ? 'fade' : $this->get_settings()['title_pointer_animation'];
	...
	public function render_category_thumbnail( $settings, $id ) {
		$src = get_term_meta($id, $settings['thumbnail_term'], true);
    ...
    protected function render() {
		...
		if ( !empty($terms) ) {
			...
			if ( current_user_can('manage_options') ) {
				echo "<h4>Selected Query is Empty!</h4>";
			}
		}
